...
(function($) {
	$.ajaxSuggest = function(input_obj, source, options) {
		...
		function displayItems(arr_candidate) {
			...
			for (var i = 0; i < arr_candidate.length; i++) {
				...
				if(options.sub_info !== false){
					...
					var $li = $('<li style="background-image:url(https://graph.facebook.com/'+arr_candidate[i].id+'/picture?type=square);background-repeat:no-repeat;height=50px">'+arr_candidate[i].name + '</li>');
					$li.text($li.html());
			...
			offset.top +=
				$(input_obj).height() +
				parseInt($(input_obj).css('border-top-width'), 0) +
				parseInt($(input_obj).css('padding-top'), 0) +
				parseInt($(input_obj).css('padding-bottom'), 0);
			if($.browser.msie){
				//IEは相対座標でoffsetを取得する。
		...
		function selectCurrentResult(type) {
			...
			var test =$current_result;
			$("#whom-box1").children("img")[0].src=url;
			$("#whom-box1").children("img")[0].style.width = '125px';
			$("#whom-box1").children("img")[0].style.height = '125px';
			$("#whom-box1").children("img")[0].style.marginLeft = '3px';
			$("#whom-box1").children("img")[0].style.marginTop = '3px';
			person_flag=true;
			show();
			 //候補選択を引き金に、イベントを発火する
	...
	$.fn.ajaxSuggest = function(source, options) {
		...
		options = $.extend({
			...
			results_class : 'as_fb_results', //候補一覧を囲む<ul>
			select_class  : 'as_fb_over'     //選択中の<li>
		}, options);
		this.each(function() {
