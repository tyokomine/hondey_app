 ...
 */
(function($) {
	$.ajaxSuggestAmazon = function(input_obj, source, options) {
		console.log(source);
		...
		//**********************************************
		var show_hide = false;
		//候補を、タイマー処理で表示するかどうかの予約
		var timer_show_hide = false;
		//タイマー。フォーカスが外れた後、候補を非表示にするか
		var timer_val_change = false;
		//タイマー変数(一定時間ごとに入力値の変化を監視)
		var reserve_click = false;
		//マウスのキーを押し続ける操作に対応するためmousedownを検知
		var $xhr = false;
		//XMLHttpオブジェクトを格納
		var prev_value = '';
		//Suggestの、以前の値
		...
		//ユーザが入力したもともとの値
		var is_select = false;
		//キー入力ではなく、候補選択で$input_objの値が変わったか？
		...
		var $results = $('<ul></ul>').addClass(options.results_class).appendTo('body');
		...
		function processKey(e) {
			if((/27$|38$|40$|^9$/.test(e.keyCode) && $results.is(':visible')) || (/^13$|^9$/.test(e.keyCode) && getCurrentResult())) {
				if(e.preventDefault)
					e.preventDefault();
		...
		function displayItems(arr_candidate) {
			console.log("表示だよ" + arr_candidate);
			arr_candidate = eval(arr_candidate);
			//候補リストを、一旦リセット
			$results.empty();
			console.log("ながさだよ" + arr_candidate.length);

			for(var i = 0; i < arr_candidate.length; i++) {
				//console.log("ながさだよ" + arr_candidate[i]);
				//候補リスト
				if(options.sub_info !== false) {
					console.log("(ﾟ∀ﾟ)ｷﾀｺﾚ!!");
					//var $li = $('<li>' + arr_candidate[i][0] + '</li>'); $$$
					//サブ情報を格納する独自の属性"sub_info"を設定
					//$li.attr('sub_info',arr_candidate[i][1]);
					var $li = $('<li>').text(arr_candidate[i][0]).attr('sub_info', arr_candidate[i][1]);
				} else {
					console.log(arr_candidate[i].SmallImage);
					if("URL" in arr_candidate[i].SmallImage) {
						var $li = $('<li style="background-image:url(' + arr_candidate[i].SmallImage.URL + ');background-repeat:no-repeat;height=50px">' + arr_candidate[i].ItemAttributes.Title + '</li>');
						$li.text($li.html());
			...
			offset.top += $(input_obj).height() + parseInt($(input_obj).css('border-top-width'), 0) + parseInt($(input_obj).css('padding-top'), 0) + parseInt($(input_obj).css('padding-bottom'), 0);
			if($.browser.msie) {
				//IEは相対座標でoffsetを取得する。
			...
			$results.show().css({
				'top' : offset.top,
				'left' : offset.left
			}).width($(input_obj).width() + parseInt($(input_obj).css('padding-left'), 10) + parseInt($(input_obj).css('padding-right'), 10));
		...
		function selectCurrentResult(type) {
			...
			$(input_obj).focus();
			var url = $current_result.css("background-image").slice(4, $current_result.css("background-image").length - 1);
			//var test = $(".book-box").children(".book1");

			if($("#book-box1").length) {
				$("#book-box1").children("img")[0].src = url;
			}
			if($(".book-box").length) {
				if($(".book-box").children(".book1")[0].src == "http://wishalist.com/cake/img/book.jpg") {
					$(".book-box").children(".book1")[0].src = url;
					$(".book-box").children(".book1")[0].style.height = '160px';
					$(".book-box").children(".book1")[0].style.width = '125px';
				} else if($(".book-box").children(".book2")[0].src == "http://wishalist.com/cake/img/book.jpg") {
					$(".book-box").children(".book2")[0].src = url;
					$(".book-box").children(".book2")[0].style.height = '160px';
					$(".book-box").children(".book2")[0].style.width = '125px';
				} else if($(".book-box").children(".book3")[0].src == "http://wishalist.com/cake/img/book.jpg") {
					$(".book-box").children(".book3")[0].src = url;
					$(".book-box").children(".book3")[0].style.height = '160px';
					$(".book-box").children(".book3")[0].style.width = '125px';
				} else if($(".book-box").children(".book4")[0].src == "http://wishalist.com/cake/img/book.jpg") {
					$(".book-box").children(".book4")[0].src = url;
					$(".book-box").children(".book4")[0].style.height = '160px';
					$(".book-box").children(".book4")[0].style.width = '125px';
				} else if($(".book-box").children(".book5")[0].src == "http://wishalist.com/cake/img/book.jpg") {
					$(".book-box").children(".book5")[0].src = url;
					$(".book-box").children(".book5")[0].style.height = '160px';
					$(".book-box").children(".book5")[0].style.width = '125px';
				}
			}
			product_flag=true;
			show();
			...
			//候補選択を引き金に、イベントを発火する
			if(options.bind_to)
				$(input_obj).trigger(options.bind_to, type);
		}
		...
		function nextResult() {
			...
			if($current_result) {
				$current_result.removeClass(options.select_class).next().addClass(options.select_class);
			} else {
		...
		function prevResult() {
			...
			if($current_result) {
				$current_result.removeClass(options.select_class).prev().addClass(options.select_class);
			} else {
	...
	// 06. 処理の始まり
	//--------------------------------------------------------------------------------
	$.fn.ajaxSuggestAmazon = function(source, options) {
		if(!source)
			return;
		console.log("(ﾟ∀ﾟ)ｷﾀｺﾚ!!");
		//************************************************************
		...
		options = $.extend({
			...
			results_class : 'as_am_results', //候補一覧を囲む<ul>
			select_class : 'as_am_over'     //選択中の<li>
		}, options);
		this.each(function() {
			new $.ajaxSuggestAmazon(this, source, options);
		});
		return this;
	};
})(jQuery);
